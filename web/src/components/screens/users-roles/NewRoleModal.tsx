...
export function NewRoleModal({
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      await clientFetch("/api/proxy/roles", {
        method: "POST",
    ...
    <Modal open={open} onClose={close} width={520}>
      <div className="p-[22px]">
        <div className="mb-1 flex items-center justify-between">
          <div className="text-[17px] font-extrabold tracking-tight">New role</div>
        ...
        <div className="mb-4">
          <Field label="NAME">
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Model Manager" />
          </Field>
        </div>
        <div className="mb-4">
